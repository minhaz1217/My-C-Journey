...
void srtf(){
    ...
    int i = 0;
    bool seen[ready_queue_rear];
    int remaining_time[ready_queue_rear], currently_waiting[ready_queue_rear];
    for(int i=0; i<ready_queue_rear; i++){
        seen[i] = false;
        remaining_time[i] = READY_QUEUE[i].getBurst();
        currently_waiting[i] = 0;
        /*
        0 means not arrived yet
        1 means waiting
        2 means terminated
        */
    }
    ...
    while(complete<ready_queue_rear){
        ...
        //No suitable process found. Incrementing Clock
        ...
        if(remaining_time[shortest] == 0){
            READY_QUEUE[shortest].setProcessStatus("Terminated");
            complete++;
            currently_waiting[shortest] = 2;
        }else{
            for(int i=0;i<ready_queue_rear;i++){
                if(seen[i] == true && READY_QUEUE[i].getProcessStatus() != "Terminated" && i!= shortest){
                    waiting_time[i]++;
                    turnaround_time[i]++;

                }
            }
            //waiting_time[shortest] = clock - READY_QUEUE[shortest].getArrivalTime() - READY_QUEUE[shortest].getBurst();
            //turnaround_time[shortest] = clock - READY_QUEUE[shortest].getArrivalTime();
...
void preemptivePriorityScheduling(){
    ...
    cout << "Priority Base Scheduling" << endl;
    //displayGanttChart(clock);
    cout << setw(30) << "Average Waiting Time: " << average(waiting_time) << endl;
    cout << setw(30) << "Average Response Time: " << average(response_time) << endl;
    cout << setw(30) << "Average Turnaround TIme: " << average(turnaround_time) << endl;
