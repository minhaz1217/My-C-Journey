...
int main(){
    int n,senderIndex =1, receiverIndex = 0,data,loopIndex = 1, arrIndex = 0,allOk,success,frameSize = 3;
    char c;
    ...
    int qSize,nextItem, rej =0, maxReceive;
    while(receiverIndex < generatedBitRange){
        ...
        for(int i=0;i<frameSize - qSize;i++){
            if(senderIndex + i + qSize < datalist.size()){
                myQ.push_back( make_pair(datalist[ senderIndex + i + qSize ], (rej+myQ.size())%frameSize ) );
                /// if the
            }
        }
