...
EWU, Bangladesh
Problem Name:
Problem Link: https://vjudge.net/problem/UVA-218
Date : 25 / August / 2019
Comment: super easy convex hull. output should be in counter clock wise and front and end same even with 1 point. had to check rotation.
*/
#include<bits/stdc++.h>
//#include<iostream>
using namespace std;
...
//        sort(vec.begin(),vec.end());
        ...
        double pere = polygonPerimeter(vec);
        if(vec.size() == 1){
            vec.push_back(vec[0]);
        }else if(vec.size()>2){
            if(ccw(vec[0],vec[1],vec[2])){
                reverse(vec.begin(),vec.end());
            }
        }
        printf("Region #%lld:\n(%.1f,%.1f)", caseCounter++, vec[0].x, vec[0].y);
        for(int i=1;i<vec.size();i++){
            printf("-(%.1f,%.1f)", vec[i].x, vec[i].y);
        }
        printf("\nPerimeter length = %.2f\n", pere);
    }
