...
EWU, Bangladesh
Problem Name:
Problem Link: https://vjudge.net/problem/UVA-361
Date : 28 / August / 2019
Comment: too many corner cases to consider. like point on line, 3 points needed.
*/
#include<bits/stdc++.h>
//#include<iostream>
using namespace std;
...
int main(){
    ...
    while(cin >> c >> r >> o){
        ...
        for(int i=0;i<o;i++){
            cin >> x >> y;
            Point p(x,y);
            if(inPolygon(p, cops) && c>=3){
                printf("     Citizen at (%.0f,%.0f) is safe.\n", x,y);
            }else if(inPolygon(p, robbers) && r>=3){
