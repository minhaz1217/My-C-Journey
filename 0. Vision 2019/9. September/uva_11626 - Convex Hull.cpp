...
EWU, Bangladesh
Problem Name:
Problem Link: https://uva.onlinejudge.org/external/116/11626.pdf
Date : 06 / September / 2019
Comment: easy convex hull.
*/
#include<bits/stdc++.h>
//#include<iostream>
using namespace std;
    ...
    while(tc--){
        ...
        for(int i = 0;i<poly.size();i++){
            if(poly[i].x < curr.x){
                curr = poly[i];
                pos = i;
            }else if(poly[i].x == curr.x && poly[i].y < curr.y){
                curr = poly[i];
                pos = i;
            }
        }
        ...
        newPoly.insert(newPoly.end(), poly.begin()+pos, poly.end());
        newPoly.insert(newPoly.end(), poly.begin(), poly.begin()+pos);
        newPoly.push_back(newPoly[0]);
        convexHull(newPoly);
        ...
        newPoly = vector<Point>();
        newPoly.insert(newPoly.end(), poly.begin()+pos, poly.end());
        newPoly.insert(newPoly.end(), poly.begin(), poly.begin()+pos);
//        for(auto it: newPoly) msg(it.x,it.y)
        cout << newPoly.size() << endl;
        for(int i=0;i<newPoly.size();i++){
            cout << newPoly[i].x << " " << newPoly[i].y << endl;
        }
    }
    return 0;
}
